			...
			if (_CP distance2D _Unit < 20) then
			{
				...
				if (count _IObjA > 0) then
				{
					...
					*/
					...
					{
						if (_x isKindOf "CAManBase" || {_x isEqualTo _IObj}) then
						{
							_list = _list - [_x];
						};
					} foreach _list;
					...
					if (!(_IObj isKindOf "CAManBase") && {!(isNull _IObj)} && {_IsCloser distance2D _FinalPoint < _IPos distance2D _FinalPoint} && {count _list isEqualTo 0} && {count _list2 isEqualTo 0}) then
					{
